...
new_choice = st.selectbox(label="Do you want to calculate a new gp, or merge with old cgpa?", options=["New", "Old"], )
if new_choice == "New":
    points, units = main()
    gp_new = points / units
    bt(gp_new)
    ...
    old_input = st.number_input(label="Enter your previous cgpa", step=1., max_value=max_val)
    old_units = st.number_input(label="Enter your previous total number of units", step=1)
